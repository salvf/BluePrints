...
public class Resizer extends MouseAdapter {
    ...
    public void mouseDragged(MouseEvent e) {
        if (thiss.getCursor() != Cursor.getDefaultCursor()) {
            Point p = e.getPoint();
            Rectangle r = thiss.rect;
            Rectangle comp = thiss.getBounds();
            int x = comp.x, y = comp.y, w = comp.width, h = comp.height;
            int type = thiss.getCursor().getType();
            int dx = p.x - r.x;
            int dy = p.y - r.y;
            switch (type) {
                ...
                case Cursor.NW_RESIZE_CURSOR:
                    ...
                    y = thiss.getBounds().y + r.y;
                    x = thiss.getBounds().x + r.x;
                    h = height;
                    w = width;
                    break;
            ...
            thiss.setBounds(x, y, w, h);
            thiss.getParent().repaint();
            thiss.getParent().getGraphics().dispose();

            dispatchParentEvent(e);
        }
    }
    ...
    private void dispatchParentEvent(MouseEvent e) {
        Component parent = SwingUtilities.getAncestorOfClass(BPViewport.class, thiss);
        if (parent instanceof BPViewport) {
            MouseEvent viewportEvent = SwingUtilities.convertMouseEvent(thiss, e, parent);
